let questions = [
  ...
  },
  {
    ...
    explanation:
      "Code signing provides a digital signature for the code, verifies the publisher of the code, and verifies that it hasn’t been modified since the publisher released it. None of the other answers verify that theapplication hasn't been modified. Input validation verifies data is valid before using it. Code obfuscation or code camouflage techniques make the code more difficult to read. Stored procedures are used with SQL databases and can be used for input validation.",
    options: [
      "A. Code signing",
      "B. Input validation",
      "C. Obfuscation",
  ...
  },
  {
    ...
    explanation:
      "Attackers use the character string ’ or'1'='1';- - in SQL injection attacks to query or modify databases. A buffer overflow attack sends more data or unexpected data to an application to access system memory. A dynamic link library (DLL) injection attack attempts to inject DLLs into memory, causing DLL commands to run. Arace condition is a programming conflict when two or more applications or application models attempt to access a resource at the same time.",
    options: [
